...
public class AddPostActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editItemName = findViewById(R.id.title);
        editDesc = findViewById(R.id.edit_description);
        editAmount = findViewById(R.id.amount1);
        editStage = findViewById(R.id.stage1);
        editcompName = findViewById(R.id.companyname1);
    ...
    private void publishPost() {
        ...
        final String itemamount = editAmount.getText().toString().trim();
        final String itemstage = editStage.getText().toString().trim();
        final String itemcompanyNmae = editcompName.getText().toString().trim();

        if(!itemName.isEmpty() && !itemDesc.isEmpty() && !itemamount.isEmpty()
                && !itemstage.isEmpty() && !itemcompanyNmae.isEmpty() && mImageUri != null) {

            final StorageReference filePath = mStorageRef.child("OpportunityImages").child(mImageUri.getLastPathSegment());
            ...
            Task<Uri> urlTask = uploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                ...
                public void onComplete(@NonNull Task<Uri> task) {
                    if (task.isSuccessful()) {
                        ...
                        newPost.child("Amount").setValue(itemamount);
                        newPost.child("Stage").setValue(itemstage);
                        newPost.child("Company Name").setValue(itemcompanyNmae);
